...
// Set Arbitrum to Sushiswap router NOT uniswap since we want to convert from sushi
// For testnet, there are no sushiswap routers so set it to our router
// UNISWAP_ROUTER.set("421611", "")
...
module.exports = async function ({ getNamedAccounts, getChainId, deployments }) {
  ...
  if (chainId != 421611 && !UNISWAP_ROUTER.has(chainId)) {
    throw Error("No Uniswap Router")
  }

  const sushiswapRouterAddress = (await deployments.get("UniswapV2Router02")).address
  const uniswapRouterAddress = chainId == 421611 ? sushiswapRouterAddress : UNISWAP_ROUTER.get(chainId);
